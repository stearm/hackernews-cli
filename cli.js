  ...
  .action(command => {
    ...
    if (command.collections) {
      command.collections.forEach(collection => {
        try {
          let collectionKeywords = fs.readFileSync(`${os.homedir()}/.hackernews-cli/${collection}`, {
            encoding: "utf-8"
          });
          ...
          console.log(
            chalk.red.bold(
              `Cannot read collection with name ${collection}, check your collections inside ${os.homedir()}/.hackernews-cli`
            )
          );
          process.exit(1);
        }
...
program
  ...
  .action(command => {
    ...
    try {
      ...
      const [collectionName, keywords] = command.split("=");
...
program
  ...
  .action(command => {
    try {
      ...
      console.log(
        chalk.red.bold(
          `Cannot read collection with name ${command}, check your collections inside ${os.homedir()}/.hackernews-cli`
        )
      );
      process.exit(1);
    }
